...
$watchdogExe = Join-Path $publishPath 'SystemHelperWatchdog.exe'
$monitorExe = Join-Path $publishPath 'SystemHelper.exe'
...
sc.exe create $ServiceName binPath= $binPath start= auto obj= LocalSystem DisplayName= "System Helper Watchdog" | Out-Null
sc.exe description $ServiceName "Restarts SystemHelper.exe if it is stopped." | Out-Null
